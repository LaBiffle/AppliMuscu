...
import { Save, Plus, Trash2, Calendar, Download, Package } from 'lucide-react-native';
...
export default function EditProgramScreen() {
  ...
  const [importExportEnabled, setImportExportEnabled] = useState(false);
  ...
  const loadProgram = async () => {
    if (typeof id === 'string') {
      ...
      if (programData) {
        ...
        setSelectedDays(programData.selectedDays);
        setDayDescriptions(programData.dayDescriptions || {});
  ...
  const loadSettings = async () => {
    ...
    setImportExportEnabled(settings.importExportEnabled);
  };
  ...
  const toggleDay = (dayIndex: number) => {
    if (selectedDays.includes(dayIndex)) {
      ...
      setDayDescriptions(prev => ({ ...prev, [dayIndex]: prev[dayIndex] || '' }));
    }
  };
  ...
  const saveProgram = async () => {
    ...
    const success = await ProgramStorage.updateProgram(program.id, updatedProgram);
  ...
  const exportProgram = () => {
    ...
    Alert.alert(
      ...
      [
        ...
        {
          text: 'Excel seul',
          onPress: () => exportExcelOnly()
        },
        {
          text: 'Archive avec images',
          onPress: () => exportWithImages()
        }
      ]
    );
  };
  ...
  const exportExcelOnly = () => {
    ...
    try {
      if (Platform.OS === 'web') {
        ExcelUtils.exportProgramAsync(program).then(() => {
          Alert.alert('Succès', 'Programme exporté avec succès');
        }).catch((error: any) => {
          Alert.alert('Erreur', 'Impossible d\'exporter le programme');
          console.error('Export error:', error);
        }).finally(() => {
          setIsExporting(false);
        });
      } else {
        ExcelUtils.exportProgram(program).then(() => {
          Alert.alert('Succès', 'Programme exporté avec succès');
        }).catch((error: any) => {
          Alert.alert('Erreur', 'Impossible d\'exporter le programme');
          console.error('Export error:', error);
  ...
  const exportWithImages = () => {
    ...
    try {
      ExcelUtils.exportProgramWithImages(program).then(() => {
        Alert.alert('Succès', 'Programme exporté avec images avec succès');
      }).catch((error: any) => {
        Alert.alert('Erreur', 'Impossible d\'exporter le programme avec images');
        console.error('Export with images error:', error);
      }).finally(() => {
        setIsExporting(false);
      });
    } catch (error) {
  ...
  return (
    <KeyboardAvoidingView 
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Modifier Programme</Text>
        <View style={styles.headerActions}>
          <TouchableOpacity style={styles.deleteButton} onPress={deleteProgram}>
