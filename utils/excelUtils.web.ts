...
export class ExcelUtils {
  static generateTemplate(): void {
    ...
    // Créer un nom de fichier avec timestamp pour éviter les conflits
    const timestamp = new Date().toISOString().slice(0, 10);
    saveAs(blob, `format_programme_sportif_${timestamp}.xlsx`);
  }

  static exportProgram(program: Program): void {
    this.exportProgramAsync(program);
  }

  static async exportProgramAsync(program: Program): Promise<void> {
    const workbook = XLSX.utils.book_new();
    ...
    const timestamp = new Date().toISOString().slice(0, 10);
    saveAs(blob, `${safeName}_${timestamp}.xlsx`);
  }

  static async importProgram(file: File): Promise<{ success: boolean; program?: Program; error?: string }> {
    try {
  ...
  static async exportWeekData(weekData: WeekData): Promise<void> {
    ...
    const excelBuffer = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });
    const blob = new Blob([excelBuffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
    const safeWeekName = weekData.weekName.replace(/[^a-zA-Z0-9\s]/g, '').replace(/\s+/g, '_');
    const safeProgramName = weekData.programName.replace(/[^a-zA-Z0-9\s]/g, '').replace(/\s+/g, '_');
    const fileName = `${safeWeekName}_${safeProgramName}.xlsx`;
  ...
  static async exportProgramWithImages(program: Program): Promise<void> {
    try {
      ...
      const excelBuffer = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });
      zip.file(`${programCopy.name.replace(/[^a-zA-Z0-9]/g, '_')}.xlsx`, excelBuffer);
  ...
  static async importProgramWithImages(file: File): Promise<{ success: boolean; program?: Program; error?: string }> {
    try {
      const zip = new JSZip();
      ...
      const excelArrayBuffer = await excelFile.async('arraybuffer');
        ...
        for (const fileName of Object.keys(zipContent.files)) {
          if (fileName.startsWith('images/') && !zipContent.files[fileName].dir) {
            try {
              ...
              // Créer un blob à partir du base64 pour le web
      ...
      selectedDays.forEach(dayIndex => {
        ...
        if (daySheet) {
          ...
          for (let i = headerRowIndex + 1; i < sheetData.length; i++) {
            ...
            if (row['Bloc'] && row['Bloc'] !== '' && row['Exercice'] && row['Exercice'] !== '') {
              // C'est une ligne d'exercice - vérifier si on doit créer un nouveau bloc
              
              if (!currentBlock || currentBlock.name !== row['Bloc']) {
                // Sauvegarder le bloc précédent s'il existe
                if (currentBlock) {
                  dayBlocks.push(currentBlock);
                }
              ...
              const exercise: Exercise = {
                id: Date.now().toString() + Math.random().toString(36).substr(2, 9),
                name: row['Exercice'] || 'Exercice sans nom',
                description: row['Description Exercice'] || '',
